...
const WriteArticle = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      const {data} = await axiosInstance.post('/api/ai/generate-article', {
        prompt,
      ...
      if (data.success) {
        ...
        toast.error(data.message || "Error generating article. Please try again.");
      } 
    } catch (error) {
    ...
    >
      ...
      <form onSubmit={onSubmitHandler} className="w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200">
        <div className="flex items-center gap-3">
          <Sparkles className="w-6 text-[#4AFAFF]" />
        ...
        <div className="mt-3 flex gap-3 flex-wrap sm:max-w-9/11">
          {
            articleLength.map((item, index) => (
              ...
              className={`text-xs px-4 py-1 border rounded-full cursor-pointer 
                ${selectedLength.text === item.text ? 'bg-teal-50 text-teal-700' 
