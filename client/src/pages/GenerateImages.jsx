...
const GenerateImages = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      const {data} = await axiosInstance.post('/api/ai/generate-image', {
        prompt,
        publish
      }, { headers: {
      ...
      if (data.success) {
        ...
        toast.error(data.message || "Error generating image. Please try again.");
      }
    } catch (error) {
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-center gap-3">
          <Image className="w-5 h-5 text-teal-300" />
