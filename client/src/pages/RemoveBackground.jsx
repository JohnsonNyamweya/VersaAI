import { Eraser, Hash, Sparkle, Sparkles } from 'lucide-react';
...
const RemoveBackground = () => {
  ...
  const onSubmitHandler = async (e) => {
      ...
      try {
        ...
        const {data} = await axiosInstance.post('/api/ai/remove-image-background', formData, { headers: {
        ...
        if (data.success) {
          ...
          toast.error(data.message || "Error removing background. Please try again.");
        }
      } catch (error) {
  ...
  return (
    ...
    >
      ...
      border border-gray-200 min-h-96 ">
        <div className="flex items-center gap-3">
          <Eraser className="w-5 h-5 text-teal-300"/>
