...
const BlogTitles = () => {
  ...
  const blogCategories = [
      'General', 'Technology', 'Business', 'Health', 'Lifestyle', 'Education', 'Food', "Travel"
    ]
    ...
    const onSubmitHandler = async (e) => {
      ...
      try {
        setLoading(true);
        const prompt = `Generate a catchy blog title about ${input} in the ${selectedCategory} category.`;
        const {data} = await axiosInstance.post('/api/ai/generate-blog-title', {
          prompt,
        ...
        if (data.success) {
          ...
          toast.error(data.message || "Error generating blog title. Please try again.");
        }
      } catch (error) {
        console.log(error?.response?.data?.message || error.message)
        toast.error("Error generating blog title. Please try again.");
      } finally {
        setLoading(false);
      }
    }
  ...
  return (
    ...
    >
        ...
        <div className="mt-3 flex gap-3 flex-wrap sm:max-w-9/11">
          {
            blogCategories.map((item) => (
              ...
              className={`text-xs px-4 py-1 border rounded-full cursor-pointer 
                ${selectedCategory === item ? 'bg-teal-50 text-teal-700' 
      ...
      border border-gray-200 min-h-96 ">
        <div className="flex items-center gap-3">
          <Hash className="w-5 h-5 text-teal-300"/>
