  ...
  const onSubmitHandler = async (e) => {
      ...
      try {
        ...
        if(object.trim().split(" ").length > 1) {
          toast.error("Please provide only a single object name to remove.");
          setLoading(false);
          return;
        }
        ...
        const {data} = await axiosInstance.post('/api/ai/remove-image-object', formData, { headers: {
        ...
        if (data.success) {
          ...
          toast.error(data.message || "Error removing object. Please try again.");
        }
      } catch (error) {
  ...
  return (
    ...
    >
      ...
      <form onSubmit={onSubmitHandler} className="w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200">
        ...
        <p className="mt-6 font-medium text-sm">Describe object name to remove</p>
        ...
        border border-gray-300"
          placeholder="eg., laptop or mouse..., Only single object name"
          required
        />
